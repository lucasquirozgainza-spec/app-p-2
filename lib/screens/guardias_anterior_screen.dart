...
/// Guardias del SISTEMA ANTERIOR (identificados por nombre). Se conserva
/// como archivo histórico: el sistema nuevo está en GuardiasScreen.
class GuardiasAnteriorScreen extends StatefulWidget {
  const GuardiasAnteriorScreen({super.key});
  @override
  State<GuardiasAnteriorScreen> createState() => _GuardiasAnteriorScreenState();
}

class _GuardiasAnteriorScreenState extends State<GuardiasAnteriorScreen> {
  ...
  Future<void> _load() async {
    ...
    final personal = await db.query('usuarios',
        where: "rol IN ('guardia','supervisor','conserje','limpieza','franquero') AND activo=1 "
            "AND guard_uuid IS NULL AND (edificio=? OR edificio IS NULL OR edificio='')",
        whereArgs: [ed],
        orderBy: 'nombre');
  ...
  /// Resumen por NOMBRE (sistema anterior; el nuevo usa el id del guardia).
  ResumenGuardia? _resumen(String edificio, String guardia) {
    for (final r in PanelHoras.porGuardia(_panel[edificio] ?? const []).values) {
      if (r.guardia == guardia) return r;
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Guardias (archivo anterior)'),
        actions: [
          IconButton(
      ...
      // Con el celular vinculado, los guardias se registran en la pantalla nueva.
      floatingActionButton: admin && !Sesion.vinculado
